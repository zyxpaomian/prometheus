...
var ( 
	...
	DefaultSDConfig = SDConfig{
        Api:    "http://localhost:8080/v1/api/scope/all",
...
func (c *SDConfig) UnmarshalYAML(unmarshal func(interface{}) error) error {
	*c = DefaultSDConfig
	...
	if len(c.Api) == 0 {
		return errors.New("restapi service discovery config must contain at least one api")
	}

    if len(c.Idc) == 0 || len(c.Env) == 0 {
        return errors.New("restapi service discovery config must contain idc and env")
    }
